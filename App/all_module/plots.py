...
def clean_dataset_name(dataset_name: str, lower: bool = True):
    dataset_name_to_display = dataset_name
    if dataset_name in ["Gaza before conflict", "Gaza during conflict", "Ukraine before conflict",
    ...
    return dataset_name, dataset_name_to_display


def generate_entity_types_plot(dataset_name: str):
    dataset_name, dataset_name_to_display = clean_dataset_name(dataset_name)
    ner_df = pd.read_csv(f"Data/Sentances_df/sentences_{dataset_name}.csv")
...
def generate_word_count_distribution_plot(dataset_name: str):
    ...
    if 'article_category_one' in df.columns:
        df = df[df['article_category_one'] != 'PHOTO']
...
def generate_community_graph(dataset_name: str):
    ...
    title = ("Nodes represent entities. Edges represent co-occurrence within the same sentence.\n"
             "Nodes size indicates the node strength.\nEdge width indicates the frequency of co-occurrence. Spring "
             "Layout")
